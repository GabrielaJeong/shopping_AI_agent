...
**재발 이유**: 검증을 typecheck/lint/HTTP 200에 의존했고, **스크롤·포커스·전환 같은 런타임/시각 동작은 흐름을 돌려봐야** 드러나는데 그 회귀 확인을 안 함.
**해결**: 스크롤 체인에 `min-h-0` 추가, 포커스는 `focus({ preventScroll: true })`.
**강화 규칙**: **레이아웃(height/overflow/flex)·포커스·시트 동작을 바꾸면, 머지 전에 전체 흐름 한 바퀴(런치→탭→푸시→시트)를 돌려 스크롤·시트 open/close·키보드(Esc/Tab)를 실제로 확인**한다(또는 그 점검을 PR 본문에 항목으로 명시). typecheck/lint/200만으론 충분치 않음.
