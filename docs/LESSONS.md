...
## L-009: 새 작업을 직전 PR의 feature 브랜치 위에서 그대로 시작함(브랜치 스택 오염)

**날짜**: 2026-06-19
**위험도**: 중간 (PR 오염 — 두 기능이 한 PR에 섞일 뻔. 커밋 전 발견해 회피)
**발생 맥락**: 찜 컬렉션(#45)을 `feature/saved-collections`에 커밋·푸시한 뒤, 곧바로 상세(Detail) 작업을 **브랜치 전환 없이 그 위에서** 편집. 5개 파일을 고치고 검증까지 한 뒤 커밋 직전에야 `git branch --show-current`로 `feature/saved-collections`임을 발견 — 그대로 커밋했으면 상세 변경이 #45(찜) PR에 섞였을 것.
**재발 이유**: PR을 푸시해도 머지 전이라 작업 브랜치에 머무는데, 다음 작업을 "새 작업"으로 의식하지 못하고 이어서 편집. L-005는 "main에 직접 커밋" 방지였고, 이건 그 반대편 — **직전 feature 브랜치에 스택**(미머지 PR 위에 다른 기능)하는 실수.
**해결**: 커밋 전 발견 → `git stash` → `git checkout main` → 새 브랜치(`feature/detail-canon`) → `git stash pop`(다른 파일이라 깨끗이 적용). #45는 오염 없이 유지.
**강화 규칙**:

1. **새 화면/기능의 첫 edit 전에 `git branch --show-current`로 "직전 feature 브랜치가 아닌지" 확인한다.** main도 직전 feature도 아닌 "이 작업 전용 새 브랜치"에서 시작. (L-005=main 직접 금지와 짝.)
2. **PR을 푸시했으면 다음 작업 시작 전 `git checkout main`(+ 필요 시 머지/pull) 후 새 브랜치를 판다.** 미머지 PR 위에 다른 기능을 쌓지 않는다.

---

## L-010: 라이브/빌드 검증을 잘못 측정해 "반영 안 됨"이라 오보 (grep 패턴 + 폴링 baseline 오류)

**날짜**: 2026-06-22
**위험도**: 중간 (오보 — 정상 배포된 걸 "배포 안 됨/자동배포 미연결"로 사용자에게 보고하고 틀린 수정 안내까지. 사용자가 지적해 정정)
**발생 맥락**: 폰트 픽스(#51, `font-mono` 제거) Vercel 배포 확인. ① 라이브 CSS에서 `grep "font-mono"`로 유틸리티 잔존을 보려 했으나, Tailwind v4 `@theme`는 **사용 여부와 무관하게 `--font-mono:` 토큰 변수를 항상 출력** → bare 문자열 "font-mono"가 그 토큰에 매칭돼 "아직 있다=구버전"으로 오판. ② 재배포 폴링 기준값(OLD)을 **이미 새 빌드로 바뀐 해시**로 잡아 "안 바뀜→TIMEOUT"으로 재오판. 결국 "자동 배포가 안 걸렸다"는 틀린 진단 + Git 연결 안내까지 했으나, 실제로는 정상 배포(`.font-mono{` 유틸=0, 옛 청크 404로 교체 확인).
**재발 이유**: L-007/L-008의 "정확히 측정" 원칙을 **grep 패턴 수준**에서 못 지킴 — 토큰 변수(`--x`)와 유틸리티 클래스(`.x{`)를 구분 안 함. 폴링 baseline을 트리거(머지) _이후_ 값으로 잡음.
**해결**: 유틸 존재는 `.<class>{`(규칙 시작)로, 토큰은 `--<name>:`로 구분해 grep. 빌드 교체는 **CSS 청크 해시 변화**(+ 옛 청크 404)로 판정. baseline은 트리거 _직전_ 값으로.
**강화 규칙**:

1. **CSS/번들에서 "이 유틸/클래스가 있나"는 bare 이름이 아니라 `.name{`(규칙 시작)으로 grep한다.** Tailwind는 미사용이어도 `--name` 토큰을 항상 내보내므로 bare 이름은 false positive.
2. **배포/빌드 교체 판정은 콘텐츠 해시(청크 URL) 변화로.** 폴링 baseline은 트리거(머지/푸시) _직전_ 값으로 캡처하고 "현재 ≠ baseline"을 본다.
3. **"반영 안 됨/안 됐다"를 보고하기 전, 측정 방법 자체(특히 grep 패턴)가 맞는지 먼저 점검.** L-007("프록시 말고 관측량")·L-008("서버 응답 확인")의 grep-레벨 연장.
