...
> **토큰 절약(읽기 규율)**: `CURRENT_STATE.md`만 통독(스냅샷). 나머지(`CHANGELOG`/`LESSONS`/`DECISIONS`/`CONVENTIONS`)는 **작업 관련 항목만 grep/부분 읽기 — 전체 통독 금지**. `CHANGELOG`는 최상단 1–2개만, 결정/교훈은 `^## D-`·`^## L-`로 헤더만 훑고 필요한 항목만 펼친다. (→ L-010 측정·검증 규율과 같은 맥락: 필요한 것만 정확히 본다)
...
- 🚩 새 작업의 첫 커밋/파일 수정 직전 → `git branch --show-current`로 **main도, 직전 feature 브랜치도 아닌지 확인**. PR 머지 후엔 main에, 푸시했지만 미머지면 직전 feature 브랜치에 머물러 있다 → 작업 시작 전 (필요 시 `git checkout main`) `git checkout -b feature/<name>` 먼저. 미머지 PR 위에 다른 기능을 쌓지 말 것. (→ L-005, L-009)
...
- 🚩 시각 변경을 했는데 **사용자 화면이 그대로**라고 할 때(특히 arbitrary Tailwind 값/토큰) → "반영됨"이라 말하기 전 **`curl`로 서버 응답(HTML 클래스 + CSS 규칙)을 직접 확인**. dev 컴파일 로그 ≠ 브라우저가 받는 것. 장기 구동 Turbopack dev는 stale 가능 → `.next` 삭제 후 재기동, 그래도 의심되면 **`pnpm build` + `start`로 확인**. 사용자와 **URL·포트·브랜치**부터 맞출 것. (→ L-008)
