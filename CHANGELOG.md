...
- 설계 결정 기록: `docs/DECISIONS.md` D-001~D-010 (스택, UI먼저·로직mock, DB=PostgreSQL, 추천 파이프라인 단계전환, 콜드스타트, 제품명 Moodyfit, Next 15 고정, 디자인 토큰=Tailwind v4 @theme, 앱 구조=단일 루트 상태 머신, 취향 벡터 구조). 각 결정에 PRD F1~F6 연결.
